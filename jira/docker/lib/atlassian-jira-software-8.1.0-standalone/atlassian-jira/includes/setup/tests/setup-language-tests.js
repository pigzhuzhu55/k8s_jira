AJS.test.require("jira.webresources:jira-setup", function () {
    ...
    test("save button should be enabled only if locale different than current is chosen", function (assert) {
        ...
        Promise.resolve().then(function () {
            // then
            equal(_this.sandbox.server.requests.length, 1, "no request made if the same locale as current selected");
            equal(_this.view.ui.button.is(":disabled"), true, "submit button is disabled");

            // when
            _this.selectLocale("de_DE");
            _this.respondToLocaleChangeWithNewI18nTexts();
            Promise.resolve().then(function () {
                // then
                equal(_this.view.ui.button.is(":disabled"), false, "submit button is enabled");

                // when
                _this.selectLocale(DEFAULT_LOCALE);
                _this.respondToLocaleChangeWithNewI18nTexts();
                Promise.resolve().then(function () {
                    // then
                    equal(_this.view.ui.button.is(":disabled"), true, "submit button is disabled");
    ...
    test("after new locale is selected, should get list of texts in new language and apply them", function (assert) {
        ...
        Promise.resolve().then(function () {
            // then
            equal(_this2.view.ui.button.text(), "common.words.save", "button has default label");
            ...
            // then
            equal(_this2.view.ui.button.text(), "button", "button's label is changed");
    ...
    test("should show a warning if cannot obtain list of texts in new language from the server", function (assert) {
        ...
        Promise.resolve().then(function () {
            ...
            _this3.respondToLocaleChangeWithError();
            Promise.resolve().then(function () {
                // then
                equal(_this3.view.ui.button[0].isBusy(), false, "spinner is hidden");
                equal(_this3.view.ui.button.is(":disabled"), true, "submit button is disabled");
                equal(_this3.view.ui.warning.length, 1, "warning is present");

                // given
                var handler = _this3.sandbox.spy();
                _this3.view.on("cancel-requested", handler);

                // when
                _this3.view.ui.cancel.click();
    ...
    test("while making request for texts language select and submit button should be disabled and spinner should be visible", function (assert) {
        ...
        Promise.resolve().then(function () {
            ...
            // then
            equal(_this4.view.langSingleSelect.$container.hasClass("aui-disabled"), true, "language select is disabled");
            equal(_this4.view.ui.button.is(":disabled"), true, "submit button is disabled");
            equal(_this4.view.ui.button[0].isBusy(), true, "footer spinner is visible");

            // when
            _this4.respondToLocaleChangeWithNewI18nTexts();
            Promise.resolve().then(function () {
                // then
                equal(_this4.view.langSingleSelect.$container.hasClass("aui-disabled"), false, "language select is enabled");
                equal(_this4.view.ui.button.is(":disabled"), false, "submit button is enabled");
                equal(_this4.view.ui.button[0].isBusy(), false, "footer spinner is hidden");
                done();
            });
        });
    });
    ...
    test("saving new locale should disable form, request the server to change the language and finally reload the page", function (assert) {
        ...
        Promise.resolve().then(function () {
            ...
            Promise.resolve().then(function () {
                ...
                // when
                var changeLanguageReq = _this5.sandbox.server.requests[2];
